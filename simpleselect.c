...
int login(struct client *p, struct client *top) {
    //char buf[256];
    //char outbuf[512];
    //sprintf("Enter your name:\n");

    char msg[] = "Enter your name:\n";
    write(p->fd, msg, strlen(msg));
    
    char name[256];
    int len = read(p->fd, name, sizeof(name) - 1);

    if (len > 0) {
        name[len] = '\0';
        char *end = strchr(name, '\n');
        if (end != NULL) {
            *end = '\0'; // Truncate at newline
        } 
        else {
        ...
        broadcast(top, broadcast_msg, strlen(broadcast_msg));
        // Add anything else?
        return 0;
    } 
    else {
...
int matching (struct client *p1, struct client *head) {
    struct client *p2;

    // Iterate to find possible opponent
    for (p2 = head; p2 != NULL; p2 = p2->next) {
        if (!p2->in_match && p2 != p1->last_opponent && p1 != p2->last_opponent) {
            // Matching criteria
            p1->in_match = 1;
            p2->in_match = 1;
            p1->last_opponent = p2;
